...
const SymbolCard: React.FC<SymbolCardProps> = ({ symbol, onClick }) => {
  ...
  return (
    ...
    >
      ...
      <div 
        ref={symbolRef}
        className={`text-4xl mb-2 ${isClient ? getSymbolClassName('symbol-large symbol-center symbol-no-select') : 'symbol-display symbol-large symbol-center symbol-no-select'}`}
      >
        {symbol.symbol}
      </div>
